...
int mmc_select_card(unsigned int flag)
{
 		...
 		if (flag & 0x04) {
	 		if(mmcinfo.sd == 1)
	 			mmc_simple_cmd(&request, SET_BUS_WIDTH, 2, RESPONSE_R1);	
	 		 	...
	 		 	mmc_simple_cmd(&request, SET_BUS_WIDTH, 0x3b70101, RESPONSE_R1);
 		
 		}
 		else if (flag & 0x01) {
 			if(mmcinfo.sd == 1)
 				mmc_simple_cmd(&request, SET_BUS_WIDTH, 0, RESPONSE_R1);
 			else
 				mmc_simple_cmd(&request, SET_BUS_WIDTH, 0x3b70001, RESPONSE_R1);
 		}		
...
static int mmc_init_card_state(struct mmc_request *request)
{
	...
	case MMC_ALL_SEND_CID:
		...
		if (mmcinfo.sd)
			...
			retval = sd_unpack_cid(request, &mmcinfo.cid);
		...
		if (mmcinfo.sd)
			mmc_simple_cmd(request, MMC_SET_RELATIVE_ADDR, 0x10,
				       RESPONSE_R6);
		else
	...
	case MMC_SEND_CSD:
		...
		if (mmcinfo.sd)
			retval = sd_unpack_csd(request, &mmcinfo.csd);
		else
