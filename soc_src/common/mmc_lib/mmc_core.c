...
int sd_unpack_csd(struct mmc_request *request, struct mmc_csd *csd)
{
	unsigned char *buf = request->response;
	int num = 0;
	if (request->result)
...
int mmc_unpack_cid(struct mmc_request *request, struct mmc_cid *cid)
{
	...
	if (buf[0] != 0x3f) {
		printf("mmc_errro_header_mismatch\n");
		return MMC_ERROR_HEADER_MISMATCH;
	}

	return 0;
}
int sd_unpack_cid(struct mmc_request *request, struct mmc_cid *cid)
{
	unsigned char *buf = request->response;
	int i;
