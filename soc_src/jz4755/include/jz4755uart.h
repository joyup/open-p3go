...
/*
// Tx
#define TRANSMIT_FIFO_EMPTY_UART(x)		( INREG8((PVOID)((x) + UART_ULSR_OFFSET)) & ULSR_TEMP )
#define UART_SEND_CHAR(x, ch)			( OUTREG8((PVOID)((x) + UART_UTHR_OFFSET), ch) )

// Rx
#define DATA_READY_UART(x)				( INREG8((PVOID)((x) + UART_ULSR_OFFSET)) & ULSR_DRY )
#define RECEIVE_CHAR_UART(x)			( INREG8((PVOID)((x) + UART_URBR_OFFSET)) )
#define RECEIVE_ERRORS_UART(x)			( INREG8((PVOID)((x) + UART_ULSR_OFFSET)) & ULSR_FIFOE )

typedef struct
{
	PVOID	pMdd;				// MDD context
	PUCHAR	pPortBase;			// Mapped virtual address of uart ports
